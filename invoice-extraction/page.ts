 ...
 *
 * Nothing here knows about invoices. The page renders whatever `reviews` the app
...
<p class=sub>Turn an invoice into structured fields, then check that the figures add up.</p>
...
<section id=step2>
  ...
  <div class=modes>
    <label class=mode><input type=radio name=mode value=schema checked>
      <span><span class=t>One record per invoice</span>
      <span class=d>The fields in <code>schema.json</code>, sent with the call. Each invoice gets its
      own record. Start here. Sent as <code>unit: per_document</code>.</span></span></label>
  ...
  <div class=field id=action-box hidden>
    ...
    <input type=text id=action placeholder="invoice-extraction">
  ...
  <div class=field>
    ...
    <textarea id=instructions rows=2 placeholder="e.g. &quot;treat a credit note as a negative amount&quot;, or &quot;shipping is an additional charge, not a line item&quot;."></textarea>
...
<section id=step3 hidden>
  ...
  <p><label class=opt><input type=checkbox id=showall>
    <span>Show fields the invoice didn't state</span></label></p>
...
function guard() {
  ...
  const needsAction = mode === 'action' && !$('action').value.trim() && !${hasAction};
...
function value(f) {
  ...
  // One marker for the field, not one per citation. A table is cited cell by
  // cell — forty entries for eight line items — and forty copies of
  // "Page 1 · Confidence 5/5" say nothing the first one did not. What differs
  // between them is worth showing: which pages, how sure the weakest is, and
  // how many there are.
  const pages = [];
