    ...
    routes: {
      ...
      "/upload": {
        POST: async (req) => {
          ...
          // The API accepts one file with `across_documents`; it just has nothing
          // to choose between, so this example asks for a second.
          if (mode === "multi-doc" && uploads.length < 2) {
            return new Response(
              "multi-doc keeps one answer where documents overlap, so it needs at least two " +
                "files. Use the schema mode for a single document.",
              { status: 400 },
            );
          }
          try {
